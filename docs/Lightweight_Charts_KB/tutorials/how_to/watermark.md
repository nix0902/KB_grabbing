# Watermark

---
source_url: https://tradingview.github.io/lightweight-charts/tutorials/how_to/watermark
---

# WatermarkLightweight Charts™ has a built-in feature for displaying simple text watermarks on your chart.
This example shows how to configure and add this simple text watermark to your chart.
If you are looking to add a more complex watermark then have a look at the [image watermark example](#image-watermark-example)
included below.
## Short answer[​](#short-answer)
A simple text watermark can be configured and added by using the [`createTextWatermark`](/lightweight-charts/docs/next/api/functions/createTextWatermark) function exported
from the library as follows:
```
import { createTextWatermark } from 'lightweight-charts';
const firstPane = chart.panes()[0];
const textWatermark = createTextWatermark(firstPane, {
horzAlign: 'center',
vertAlign: 'center',
lines: [
{
text: 'Watermark Example',
color: 'rgba(171, 71, 188, 0.5)',
fontSize: 24,
},
],
});
```
The options available for the watermark are: [TextWatermark Options](/lightweight-charts/docs/next/api/interfaces/TextWatermarkOptions).
You can see full [working examples](#examples) below.

## Image Descriptions

### Simple Text Watermark Chart
An area chart demonstrating a simple text watermark centered on the chart. The chart features:
- **Chart Type**: Area series with blue line (#4285F4) and semi-transparent blue gradient fill
- **Background**: White with light gray horizontal grid lines
- **Y-Axis (Price Scale)**: Vertical axis on the right showing values from 0.00 to 50.00
- **X-Axis**: Time-based showing integer values (17, 19, 21, 23, 25)
- **Data Pattern**: Step-like upward trend with plateau phases, rising from ~0 to ~46
- **Watermark**: Semi-transparent purple text "Watermark Example" centered on the chart
  - **Color**: rgba(171, 71, 188, 0.5) - semi-transparent purple/magenta
  - **Position**: Center-aligned both horizontally and vertically
  - **Font Size**: 24px
  - **Style**: Bold, unobtrusive overlay that doesn't interfere with data visibility
- **Current Value**: Blue rectangular tooltip showing "46.00" at the rightmost data point
- **Purpose**: Demonstrates the built-in `createTextWatermark` function for adding branding or context to charts

### Image Watermark Chart
An area chart demonstrating an image watermark with a logo. The chart features:
- **Chart Type**: Area series with blue line and gradient fill
- **Background**: White with light gray grid lines
- **Y-Axis**: Vertical axis on the right
- **X-Axis**: Time-based
- **Watermark**: TradingView logo image watermark centered on the chart
  - **Image**: Lightweight Charts by TradingView logo (black text on transparent background)
  - **Alpha**: 0.5 (50% transparency)
  - **Padding**: 20px around the image
  - **Position**: Center-aligned
- **Purpose**: Demonstrates the `createImageWatermark` function for adding complex watermarks like logos
- **Note**: Image watermarks allow for more complex branding than text watermarks

## Resources[​](#resources)
- [`createTextWatermark` function](/lightweight-charts/docs/next/api/functions/createTextWatermark).
- [TextWatermark Options](/lightweight-charts/docs/next/api/interfaces/TextWatermarkOptions)
## Examples[​](#examples)
How to use the code sample
**The code presented below requires:**
- That `createChart` has already been imported. See [Getting Started](/lightweight-charts/docs#creating-a-chart) for more information,
- and that there is an html div element on the page with an `id` of `container`.
Here is an example skeleton setup: [Code Sandbox](https://codesandbox.io/s/lightweight-charts-skeleton-n67pm6).
You can paste the provided code below the `// REPLACE EVERYTHING BELOW HERE` comment.
tip
Some code may be hidden to improve readability. Toggle the checkbox above the code block to reveal all the code.
### Simple Watermark Example[​](#simple-watermark-example)
Show all code
```
// Lightweight Charts™ Example: Watermark Simple
// https://tradingview.github.io/lightweight-charts/tutorials/how_to/watermark
const chartOptions = {
layout: {
textColor: 'black',
background: { type: 'solid', color: 'white' },
},
};
/** @type {import('lightweight-charts').IChartApi} */
const chart = createChart(document.getElementById('container'), chartOptions);
/** @type {import('lightweight-charts').createTextWatermark} */
createTextWatermark(chart.panes()[0], {
horzAlign: 'center',
vertAlign: 'center',
lines: [
{
text: 'Watermark Example',
color: 'rgba(171, 71, 188, 0.5)',
fontSize: 24,
},
],
});
const lineSeries = chart.addSeries(AreaSeries, {
topColor: '#2962FF',
bottomColor: 'rgba(41, 98, 255, 0.28)',
lineColor: '#2962FF',
lineWidth: 2,
});
const data = [
{ value: 0, time: 1642425322 },
{ value: 8, time: 1642511722 },
{ value: 10, time: 1642598122 },
{ value: 20, time: 1642684522 },
{ value: 3, time: 1642770922 },
{ value: 43, time: 1642857322 },
{ value: 41, time: 1642943722 },
{ value: 43, time: 1643030122 },
{ value: 56, time: 1643116522 },
{ value: 46, time: 1643202922 },
];
lineSeries.setData(data);
chart.timeScale().fitContent();
```
### Image Watermark Example[​](#image-watermark-example)
If a simple text watermark doesn't meet your requirements then you can use the Image watermark via [`createImageWatermark`](/lightweight-charts/docs/next/api/functions/createImageWatermark) function exported
from the library as follows:
```
import { createImageWatermark } from 'lightweight-charts';
const firstPane = chart.panes()[0];
const imageWatermark = createImageWatermark(firstPane, '/images/my-image.png', {
alpha: 0.5,
padding: 20,
});
```
The options available for the watermark are: [ImageWatermark Options](/lightweight-charts/docs/next/api/interfaces/ImageWatermarkOptions).
You can see full [working examples](#examples) below.
## Resources[​](#resources-1)
- [`createImageWatermark` pane primitive](/lightweight-charts/docs/next/api/functions/createTextWatermark).
- [ImageWatermark Options](/lightweight-charts/docs/next/api/interfaces/ImageWatermarkOptions)
tip
Since the watermark image is black content with a transparent background, it may not be visible when
viewing the documentation site in dark mode.
Show all code
```
// Lightweight Charts™ Example: Image Watermark
// https://tradingview.github.io/lightweight-charts/tutorials/how_to/watermark
const chartOptions = {
layout: {
textColor: 'black',
background: { type: 'solid', color: 'white' },
},
};
/** @type {import('lightweight-charts').IChartApi} */
const chart = createChart(document.getElementById('container'), chartOptions);
// imageDataUrl would usually be an url like '/images/my-image.png'
const imageDataUrl = 'data:image/svg+xml;base64,';
createImageWatermark(chart.panes()[0], imageDataUrl, {
alpha: 0.5,
padding: 20,
});
const lineSeries = chart.addSeries(AreaSeries, {
topColor: '#2962FF',
bottomColor: 'rgba(41, 98, 255, 0.28)',
lineColor: '#2962FF',
lineWidth: 2,
});
const data = [
{ value: 0, time: 1642425322 },
{ value: 8, time: 1642511722 },
{ value: 10, time: 1642598122 },
{ value: 20, time: 1642684522 },
{ value: 3, time: 1642770922 },
{ value: 43, time: 1642857322 },
{ value: 41, time: 1642943722 },
{ value: 43, time: 1643030122 },
{ value: 56, time: 1643116522 },
{ value: 46, time: 1643202922 },
];
lineSeries.setData(data);
chart.timeScale().fitContent();
```
