...
---
source_url: https://tradingview.github.io/lightweight-charts/tutorials/how_to/inverted-price-scale
---

# Inverted Price ScaleThis example shows how to invert a price scale. Usually, the price scale will
...
top to bottom.
## How to[​](#how-to)
Set the [`invertScale`](/lightweight-charts/docs/api/interfaces/PriceScaleOptions#invertscale) property
on the [priceScale options](/lightweight-charts/docs/api/interfaces/PriceScaleOptions) to `true`.
```
chart.applyOptions({
rightPriceScale: {
invertScale: true,
...
### Inverted Price Scale Chart
A line chart demonstrating an inverted price scale where lower values appear at the top and higher values at the bottom. The chart features:
- **Chart Type**: Single line series with a blue solid line (#0071c5)
- **Background**: White with light gray horizontal grid lines
- **Y-Axis (Price Scale)**: Vertical axis on the right showing values from 0.00 to 20,000.00 in increments of 4,000.00 - INVERTED (high values at bottom, low values at top)
- **X-Axis**: Time-based (not explicitly labeled)
- **Data Pattern**: Line starts high (near 20,000), trends downward with fluctuations, shows a sharp drop to a low around 8,000, then recovers with volatility, ending at 8,702.43
- **Reference Line**: Horizontal dashed light blue line marking a mid-range reference level
- **Current Value**: Blue rectangular tooltip at bottom-right showing "8,702.43"
- **Purpose**: Demonstrates how to invert the price scale for specific charting needs, useful for certain financial instruments or analytical perspectives

### Resources[​](#resources)
- [invertScale](/lightweight-charts/docs/api/interfaces/PriceScaleOptions#invertscale)
- [Price Scales](/lightweight-charts/docs/price-scale) - General introduction to Price Scales.
