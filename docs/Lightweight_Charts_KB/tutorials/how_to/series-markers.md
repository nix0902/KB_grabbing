...
---
source_url: https://tradingview.github.io/lightweight-charts/tutorials/how_to/series-markers
---

# Add Series MarkersA series marker is an annotation which can be drawn on the chart at a specific point.
...
You can add markers to a series by passing an array of [`seriesMarker`](/lightweight-charts/docs/api/type-aliases/SeriesMarker)
objects to the [`createSeriesMarkers`](/lightweight-charts/docs/next/api/functions/createSeriesMarkers) method on
an [`ISeriesApi`](/lightweight-charts/docs/api/interfaces/ISeriesApi) instance.
```
const markers = [
{
...
### Series Markers Chart
A candlestick chart demonstrating the use of series markers to annotate specific data points. The chart features:
- **Chart Type**: Candlestick series with alternating green and red candles
- **Candle Colors**: Green (#26a69a) for bullish candles, red (#ef5350) for bearish candles
- **Background**: White with light gray horizontal grid lines
- **Y-Axis (Price Scale)**: Vertical axis on the right showing values from 0.00 to 120.00
- **X-Axis**: Time-based showing date progression
- **Data Pattern**: Upward trend starting around 20.00, rising to peak near 110.00, with volatility throughout
- **Series Markers**: Three annotation markers displayed on the chart:
  - **Circle Marker** (orange, #f68410): Positioned above a candlestick at the beginning of the trend, labeled "D"
  - **Arrow Up Marker** (blue, #2196F3): Positioned below a candlestick showing "Buy @ 52" with upward arrow shape
  - **Arrow Down Marker** (pink, #e91e63): Positioned above a candlestick showing "Sell @ 75" with downward arrow shape
- **Current Value**: Blue rectangular tooltip at bottom-right showing "72.91"
- **Purpose**: Demonstrates how to add visual annotations (circles, arrows, squares) to highlight buy/sell signals or important price levels
- **Marker Positions**: `aboveBar` for sell signal, `belowBar` for buy signal, allowing markers to not overlap with candlesticks
...
`low` in the case of candlestick data) and the specified `position` property ([SeriesMarkerPosition](/lightweight-charts/docs/api/type-aliases/SeriesMarkerPosition)).
## Resources[​](#resources)
You can view the related APIs here:
- [SeriesMarker](/lightweight-charts/docs/api/type-aliases/SeriesMarker) - Series Marker interface.
- [SeriesMarkerPosition](/lightweight-charts/docs/api/type-aliases/SeriesMarkerPosition) - Positions that can be set for the marker.
- [SeriesMarkerShape](/lightweight-charts/docs/api/type-aliases/SeriesMarkerShape) - Shapes that can be set for the marker.
- [createSeriesMarkers](/lightweight-charts/docs/next/api/functions/createSeriesMarkers) - Method for adding markers to a series.
- [Time Types](/lightweight-charts/docs/api/type-aliases/Time) - Different time formats available to use.
