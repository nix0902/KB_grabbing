...
---
source_url: https://tradingview.github.io/lightweight-charts/tutorials/how_to/tooltips
---
...
### Floating Tooltip Chart
An area chart demonstrating a custom floating tooltip that follows the data point. The chart features:
- **Chart Type**: Area series with blue line (#6495ED) and semi-transparent blue gradient fill
- **Background**: White with no visible grid lines (hidden for cleaner tooltip display)
- **Y-Axis (Price Scale)**: Vertical axis on the right showing values from 25.00 to 26.50
- **X-Axis**: Time-based (not explicitly labeled)
- **Data Pattern**: Volatile movement starting at 26.19, with peak at 26.23, dip to 25.35, recovery to 26.23
- **Custom Tooltip**: Floating white rectangular box with blue border (#2962FF) positioned above the chart:
  - **Header**: "Apple Inc." in blue text
  - **Price Value**: Large "26.23" in black (24px font)
  - **Date**: "2019-05-28" in black text below
- **Current Value**: Blue rectangular tooltip on right showing "26.23"
- **Purpose**: Demonstrates how to create custom HTML tooltips that follow data points or cursor position

### Tracking Tooltip Chart
An area chart demonstrating a tracking tooltip that follows the cursor. The chart features:
- **Chart Type**: Area series with teal line (#26a69a) and semi-transparent teal gradient fill
- **Background**: White with no visible grid lines
- **Y-Axis (Price Scale)**: Vertical axis on the right
- **X-Axis**: Time-based
- **Tooltip Behavior**: Follows the mouse cursor position instead of snapping to data points
- **Purpose**: Shows how to create tooltips that track mouse movement

### Magnifier Tooltip Chart
An area chart demonstrating a magnifier tooltip positioned at the top of the chart. The chart features:
- **Chart Type**: Area series with red line (#ef5350) and gradient fill
- **Background**: White with no visible grid lines
- **Y-Axis (Price Scale)**: Vertical axis on the LEFT side (unusual positioning)
- **X-Axis**: Time-based with visible dates
- **Tooltip Position**: Fixed at top of chart, follows horizontal time position only
- **Purpose**: Demonstrates a fixed-position tooltip style commonly used for detailed data inspection

### Getting the mouse cursors position[​](#getting-the-mouse-cursors-position)
...
### Getting the data points position[​](#getting-the-data-points-position)
...
```
### Tracking Tooltip[​](#tracking-tooltip)
...
chart.timeScale().fitContent();
```
### Magnifier Tooltip[​](#magnifier-tooltip)
