...
---
source_url: https://tradingview.github.io/lightweight-charts/tutorials/how_to/legends
---
...
A line chart with area fill demonstrating a custom legend implementation. The chart features:
- **Chart Type**: Area series with a blue line (#4285F4) and semi-transparent blue gradient fill beneath
- **Legend Display**: "ETC USD 7D VWAP" text label with dynamic price value at the top-left of the chart
- **Background**: White with no visible grid lines (grid hidden for cleaner legend display)
- **Y-Axis (Price Scale)**: Vertical axis on the right showing values from 25.00 to 26.50 in 0.50 increments
- **X-Axis**: Time-based (not explicitly labeled)
- **Data Pattern**: Line shows moderate volatility - starts at 26.19, peaks at 26.23, dips to around 25.35, and ends at approximately 26.23
- **Current Value**: Blue rectangular tooltip on right showing "26.23"
- **Legend Style**: Black text showing "ETC USD 7D VWAP" with bold price value, positioned at top-left inside chart area
- **Purpose**: Demonstrates how to create custom HTML legends that update dynamically with crosshair movement
...
### Simple Legend Example[​](#simple-legend-example)
Show all code
```
...
```
### 3 Line Legend Example[​](#3-line-legend-example)
Show all code
```
