...
---
source_url: https://tradingview.github.io/lightweight-charts/tutorials/how_to/price-and-volume
---

# Price and volume on a single chart
This example shows how to include a volume study on your chart.
## How to add a volume histogram[​](#how-to-add-a-volume-histogram)
An additional series can be added to a chart as an 'overlay' by setting the series'
[`priceScaleId`](/lightweight-charts/docs/api/interfaces/SeriesOptionsCommon#pricescaleid) to `''`.
An overlay doesn't make use of either the left or right price scale, and it's positioning
is controlled by setting the [`scaleMargins`](/lightweight-charts/docs/api/interfaces/PriceScaleOptions#scalemargins)
...
```
We are using the [Histogram](/lightweight-charts/docs/series-types#histogram) series type to draw the volume bars.
...
setting the [`scaleMargins`](/lightweight-charts/docs/api/interfaces/PriceScaleOptions#scalemargins) properties as follows:
```
volumeSeries.priceScale().applyOptions({
scaleMargins: {
top: 0.7, // highest point of the series will be 70% away from the top
...
### Price and Volume Overlay Chart
A combined chart demonstrating price and volume displayed in a single chart area with overlay positioning. The chart features:
- **Top Section (Price - Area Chart)**:
  - **Chart Type**: Area series with blue line (#4285F4) and semi-transparent blue gradient fill
  - **Y-Axis**: Price scale on the right showing values from 54.00 to 60.00
  - **Grid Lines**: Light gray horizontal grid lines
  - **Data Pattern**: Gradual upward trend from ~55.00 to ~59.00 with minor fluctuations
  - **Current Price**: Blue rectangular tooltip showing "59.57"
- **Bottom Section (Volume - Histogram)**:
  - **Chart Type**: Histogram series with vertical bars
  - **Bar Colors**: Green (#34A853) for up-volume, red (#EA4335) for down-volume
  - **Positioning**: Volume bars occupy the bottom 30% of the chart area using `scaleMargins`
  - **Values**: Total volume shown as "3.1M" on the right axis
  - **Data Pattern**: Variable volume with occasional spikes correlating with price movements
- **Purpose**: Demonstrates how to overlay volume on a price chart using `priceScaleId: ''` and custom `scaleMargins`
- **Key Concept**: Both series share the same time axis but have independent price scales and vertical positioning

## Resources[​](#resources)
- [OverlayPriceScale Options](/lightweight-charts/docs/api/type-aliases/OverlayPriceScaleOptions)
- [Histogram Series Type](/lightweight-charts/docs/series-types#histogram)
- [PriceFormat Types](/lightweight-charts/docs/api/interfaces/PriceFormatBuiltIn#type)
- [Scale Margins](/lightweight-charts/docs/api/interfaces/PriceScaleOptions#scalemargins)
