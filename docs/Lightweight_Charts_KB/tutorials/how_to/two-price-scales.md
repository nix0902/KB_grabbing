...
---
source_url: https://tradingview.github.io/lightweight-charts/tutorials/how_to/two-price-scales
---

# Two Price ScalesIt is possible to have two price scales visible on a Lightweight Charts™,
...
set to `true` within the [chart options](/lightweight-charts/docs/api/interfaces/ChartOptionsBase#leftpricescale).
```
chart.applyOptions({
rightPriceScale: {
visible: true,
...
```
and assign the `priceScaleId` property on the [series options](/lightweight-charts/docs/api/interfaces/SeriesOptionsCommon#pricescaleid)
...
### Two Price Scales Chart
A combined chart demonstrating the use of dual price scales (left and right). The chart features:
- **Chart Type**: Two series sharing the same time axis but using different price scales
- **Background**: White with light gray horizontal and vertical grid lines
- **Left Y-Axis (Price Scale)**: 
  - Vertical axis on the LEFT side showing values from 20.00 to 120.00
  - Used by the candlestick series
- **Right Y-Axis (Price Scale)**:
  - Vertical axis on the RIGHT side showing values from 20.00 to 100.00
  - Used by the line series
- **X-Axis**: Time-based showing date progression (Sep 22 - Dec 31)
- **Line Series**:
  - Blue solid line (#2962FF)
  - Data pattern: Upward trend with volatility from ~25 to ~100
  - Uses the RIGHT price scale
- **Candlestick Series**:
  - Green candles (#26a69a) for bullish, red candles (#ef5350) for bearish
  - Data pattern: Volatile price action with wicks showing high/low ranges
  - Uses the LEFT price scale (configured with `priceScaleId: 'left'`)
- **Crosshair Mode**: Normal mode (allows crosshair to sit directly under cursor instead of snapping to data points)
- **Purpose**: Demonstrates how to display two series with different value ranges on the same chart using separate price scales
- **Key Concept**: Each series can be assigned to a specific price scale using `priceScaleId`, allowing for comparison of instruments with different price ranges

## Tips[​](#tips)
By default the crosshair will snap to the data points of the first series.
You may prefer to set the [crosshair mode](/lightweight-charts/docs/api/enumerations/CrosshairMode) to
...
```
## Resources[​](#resources)
You can learn more about price scales here: [Price scale](/lightweight-charts/docs/price-scale)
and view the related APIs here:
- [Chart Options](/lightweight-charts/docs/api/interfaces/ChartOptionsBase#leftpricescale)
- [PriceScaleOptions](/lightweight-charts/docs/api/interfaces/PriceScaleOptions)
- [SeriesOptionsCommon priceScaleId](/lightweight-charts/docs/api/interfaces/SeriesOptionsCommon#pricescaleid)
