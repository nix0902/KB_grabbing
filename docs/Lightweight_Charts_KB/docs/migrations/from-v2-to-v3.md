...
You can refer to the new API [here](https://tradingview.github.io/lightweight-charts/docs/api).
...
| v2 API | v3 API |
|--------|--------|
| `chart.subscribeVisibleTimeRangeChange` | `chart.timeScale().subscribeVisibleTimeRangeChange` |
| `chart.unsubscribeVisibleTimeRangeChange` | `chart.timeScale().unsubscribeVisibleTimeRangeChange` |
| `priceScale: { position: 'left' }` | `leftPriceScale: { visible: true }`, `rightPriceScale: { visible: false }` |
| `priceScale: { position: 'none' }` | `leftPriceScale: { visible: false }`, `rightPriceScale: { visible: false }` |
| `overlay: true` (series option) | `priceScaleId: ''` |
