...
- / [ Directional Movement (DMI) ](/support/solutions/43000502250-directional-movement-dmi/)
...
If DownMove > Upmove and Downmove > 0, then -DM = DownMove, else -DM = 0 Once you have the current +DM and -DM calculated, the +DM and -DM lines can be calculated and plotted based on the number of user defined periods.

+DI = 100 times Exponential Moving Average of (+DM / Average True Range)
-DI = 100 times Exponential Moving Average of (-DM / Average True Range) Now that -+DX and -DX have been calculated, the last step is calculating the ADX.
...
Sets the number of decimal places to be left on the indicator's value before rounding up. The higher this number, the more decimal points will be on the indicator's value.
 Previous Previous Detrended Price Oscillator (DPO) Next Next Divergence Launch Supercharts
...
**URL:** https://s3.amazonaws.com/cdn.freshdesk.com/data/helpdesk/attachments/production/43586538078/original/YlJ3v_a7vpCRuf9lRii3zNXnuGH75TR-uA.png?1760615956

**Описание:**

Error: Command '['z-ai', 'vision', '-p', 'Describe this TradingView interface image in detail. Explain what it shows, all UI elements, buttons, and their purposes.', '-i', 'https://s3.amazonaws.com/cdn.freshdesk.com/data/helpdesk/attachments/production/43586538078/original/YlJ3v_a7vpCRuf9lRii3zNXnuGH75TR-uA.png?1760615956']' timed out after 90 seconds

---

### Изображение 2

**URL:** https://s3.amazonaws.com/cdn.freshdesk.com/data/helpdesk/attachments/production/43586538155/original/q34qbIR2_BnhJlwXCPsA12qgepXQvLJgHw.png?1760615973

**Описание:**

This image shows a **TradingView chart interface** displaying Apple Inc. (NASDAQ: AAPL) stock data with a **DMI (Directional Movement Index) indicator configuration modal** open. Here’s a detailed breakdown:  


### 1. **Top Navigation Bar**  
- **Left Side**:  
  - `AAPL` (stock ticker) with a star icon (favorite/track).  
  - `+` (add new chart), `D` (timeframe selector, currently “1D” = daily), `⚖️` (volume), `Indicators` (dropdown to add indicators), `W` (workspace), `Alert` (set price alerts), `Replay` (replay market movements), and navigation arrows (back/forward).  
- **Right Side**:  
  - `TradingView` (logo/dropdown), `M`/`M`/`S` (market/market depth/symbol info), refresh, settings, chart type, camera, and `Publish` (share chart).  


### 2. **Chart Header (Stock Info)**  
- Ticker: `Apple Inc · 1D · NASDAQ`  
- Price data: `O 249.49 H 251.82 L 247.47 C 249.34 +1.57 (+0.63%)` (Open, High, Low, Close, change).  
- Currency: `USD` (dropdown for currency selection).  


### 3. **Main Chart Area**  
- **Candlestick Chart**: Shows Apple’s daily price action (green = up, red = down) over months (May–Nov).  
- **DMI Indicator (Below Main Chart)**: Three lines (ADX, +DI, -DI) with colored markers (red, blue, orange) and values (e.g., `ADX 28.3421`, `+DI 24.4574`, `-DI 22.4312`).  
...
- Text, zoom, magnet (snap to price),  
- Lock (prevent edits), visibility, globe (language), trash (delete), star (favorites).  


### 5. **DMI Configuration Modal (Center)**  
A popup for customizing the DMI indicator, with three tabs:  

#### **Tabs**:  
- `Inputs` (settings for DMI parameters),  
- `Style` (**active tab**, for visual customization),  
- `Visibility` (toggle indicator display).  

#### **Style Tab Elements**:  
- **Checkboxes**:  
  - `ADX` (checked, red line), `+DI` (checked, blue line), `-DI` (checked, orange line).  
- **Color/Line Style**:  
  - Color swatches (red, blue, orange) and line-style toggles (solid/dashed).  
- **Output Values**:  
  - `Precision`: Dropdown (set to “Default”).  
  - `Labels on price scale` (checked, show values on price axis),  
  - `Values in status line` (checked, show values in chart status bar).  
- **Buttons**:  
  - `Defaults` (reset to default settings),  
  - `Cancel` (close modal without saving),  
  - `Ok` (apply changes and close modal).  


### 6. **Bottom Toolbar**  
- **Timeframe Selector**: `1D 5D 1M 3M 6M YTD 1Y 5Y All` (switch chart periods).  
- **Date Range**: `May–Nov` (x-axis labels).  
- **Timestamp**: `14:57:13 UTC+3` (current time).  
- **Additional**: `ADJ` (adjusted data toggle), `Pine Editor`/`Trading Panel` (Pine Script editor, trading tools).  


### 7. **Right Sidebar (Icons)**  
Icons for:  
- Notes, clock (time), patterns, chat, target/stop, alerts, calendar, grid, signals, help.  


### Purpose of Key Elements:  
- **DMI Modal**: Customize the Directional Movement Index (ADX, +DI, -DI) to analyze trend strength/direction.  
- **Timeframe Selector**: View price data over different periods (1 day, 5 days, 1 month, etc.).  
- **Indicators Dropdown**: Add technical indicators (e.g., moving averages, RSI) to the chart.  
- **Drawing Tools**: Annotate charts (trendlines, support/resistance).  


This interface is used for **technical analysis**—traders analyze price trends, customize indicators, and manage charts to make informed trading decisions.
